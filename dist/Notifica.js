...
function notificaLocaciones1(rowData) {
    ...
    try {
        ...
        if (isChecked !== true && correoSecretario && nombreLocador && secretario) {
            ...
            GmailApp.sendEmail(correoSecretario, ASUNTO_NOTIFICACION, "Este es un mensaje de texto plano de respaldo...", options);
            ...
            if (isChecked === true)
                errorMessage += "El checkbox ya está marcado. ";
            if (!correoSecretario)
                errorMessage += "Correo del Secretario vacío. ";
            if (!nombreLocador)
                errorMessage += "Nombre del Locador vacío. ";
            if (!secretario)
                errorMessage += "Nombre del Secretario vacío. ";
            Logger.log(errorMessage);
            return { success: false, message: errorMessage };
        }
    }
