 ...
 */
function updateTareaConfig(oldName, newName) {
    try {
 ...
 */
function deleteTareaConfig(tareaName) {
    try {
...
function getData(filterOptions = { type: 'recent' }) {
    try {
        ...
        if (altaIdx === -1)
            altaIdx = allHeaders.indexOf('FECHA DE ALTA');
        ...
        for (let i = 0; i < rawData.length; i++) {
            ...
            if (filterOptions.type !== 'all' && altaIdx !== -1) {
                ...
                if (filterOptions.type === 'recent') {
                    if (isDateValid) {
                        ...
                        if (!((y === anioActual && m === mesActual) || (y === anioAnterior && m === mesAnterior)))
                            continue;
                    }
                }
            ...
            allHeaders.forEach((header, index) => {
                if (header) {
                    ...
                    if (header === 'FECHA DE ALTA')
                        rowObject['FECHA ALTA'] = row[index];
                    if (header === 'FECHA DE BAJA')
                        rowObject['FECHA BAJA'] = row[index];
                }
            });
            try {
...
function checkDataVersion(filterOptions) {
    try {
        ...
        const lastRow = sheet.getLastRow();
        if (lastRow <= 1)
            return { success: true, version: "empty" };
