...
function generaResolucionTXT(rowData) {
    ...
    try {
        ...
        if (!idPlantillaResolucion)
            throw new Error("ID de plantilla no configurado (DOC_TEMPLATE_RESOLUCION_ID).");
        if (!idCarpetaResolucion)
            throw new Error("ID de carpeta no configurado (CARPETA_FUSION_RESOLUCIONES).");
...
function bajaResolucionTXT(rowData) {
    ...
    try {
        ...
        // 2. Validar acceso a plantilla y carpeta
        if (!docTemplateBajaResolucionId)
            throw new Error("ID de plantilla de baja no configurado (DOC_TEMPLATE_BAJA_RESOLUCION_ID).");
        if (!CARPETA_FUSION_RESOLUCIONES)
            throw new Error("ID de carpeta no configurado (CARPETA_FUSION_RESOLUCIONES).");
