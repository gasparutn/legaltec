    ...
    try {
        ...
        for (const u of todosUsuarios) {
            ...
            if (usr === usrInput && pw === pwInput) {
                ...
                if (!status)
                    return { ok: false, message: 'Usuario inhabilitado por un supervisor.' };
                const userRol = String(rol).trim().toLowerCase();
                // --- LOGICA 2FA PARA ADMIN ---
                if (userRol === 'admin') {
                    if (!emailUser)
                        return { ok: false, message: 'Error: El administrador no tiene un correo configurado.' };
 ...
 */
function verifyTokenAdmin(uid, token) {
    const cache = CacheService.getScriptCache();
    ...
    if (!admin)
        return { ok: false, message: 'Administrador no encontrado en Firebase.' };
    return {
        ok: true,
        rol: 'admin',
 ...
 */
function crearUsuarioCD(nombreRaw, usuarioRaw, pass, solicitanteRol, autorCod) {
    try {
 ...
 * Solo pueden ejecutarlo admin o supervisor.
 */
function listarUsuariosCD(solicitanteRol) {
    try {
        if (solicitanteRol !== 'admin' && solicitanteRol !== 'supervisor') {
 ...
 */
function cambiarStatusUsuarioCD(idUsuario, nuevoStatus, solicitanteRol) {
    try {
        if (solicitanteRol !== 'admin' && solicitanteRol !== 'supervisor') {
 ...
 */
function eliminarUsuarioCD(idUsuario, solicitanteRol) {
    try {
        if (solicitanteRol !== 'admin' && solicitanteRol !== 'supervisor') {
        ...
        const user = firestoreGetDoc("Usuarios", idUsuario);
        if (!user)
            return { ok: false, message: 'Usuario con ID ' + idUsuario + ' no encontrado.' };
        ...
        try {
            ...
            Logger.log("Error limpiando chat: " + e.message);
        }
...
function crearRequerimiento(apellidoNombreRaw, autoridadRaw, usuarioId) {
    try {
        const uid = usuarioId;
        const apellidoNombre = sanitizeInput(apellidoNombreRaw).toUpperCase();
        const autoridad = sanitizeInput(autoridadRaw);
        if (!uid)
            return { ok: false, message: 'Identificador de usuario faltante.' };
        ...
        for (const d of todosDocs) {
            ...
            if (!isNaN(idNum) && idNum > lastDocId)
                lastDocId = idNum;
        }
        for (let i = 1; i <= 15; i++) {
...
function listarRequerimientos(usuarioId, rol) {
    try {
        ...
        if (!uid)
            return { ok: false, message: 'Datos de sesión incompletos.' };
        const todosReqs = firestoreGetAllDocs("Requerimientos");
        const todosDocs = firestoreGetAllDocs("Documentos");
        const todosUsuarios = firestoreGetAllDocs("Usuarios");
...
function subirDocumentoLocalCD(idDoc, mimeType, base64Data, fileName) {
    try {
        ...
        if (!doc)
            return { ok: false, message: 'Documento no encontrado en Firebase.' };
...
function aprobarDocumento(idDoc, nombreAprobador) {
    try {
...
function aprobarDocumentosLote(idDocs, nombreAprobador) {
    try {
        const fechaAprobacion = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'dd/MM/yyyy HH:mm');
        const reqsParaChequear = new Set();
...
function _checkCompletitudReqFirestore(idReq) {
    try {
        const todosDocs = firestoreGetAllDocs("Documentos");
...
function rechazarDocumento(idDoc, motivoRaw) {
    try {
...
function eliminarRequerimientoComp(idReq, uid, rol) {
    try {
        ...
        const req = firestoreGetDoc("Requerimientos", idReq);
        if (!req)
            return { ok: false, message: 'Requerimiento no encontrado en Firebase.' };
 ...
 */
async function generarPdfCD(idReq, nombreAprobador) {
    try {
        ...
        for (const d of docs) {
            if (d.urlDrive) {
                try {
                    ...
                    if (match)
                        pdfBlobs.push(DriveApp.getFileById(match[0]).getBlob());
                }
                catch (e) { }
            }
        }
 ...
 */
function chatSyncCD(uid, rol, nombre, lastMsgId, ghostMode = false, targetUid) {
    try {
        const sp = PropertiesService.getScriptProperties();
        const cache = CacheService.getScriptCache();
        const now = Date.now();
        ...
        try {
            onlineDict = JSON.parse(onlineStr);
        }
        catch (e) { }
        if (!lastUserSync || (now - Number(lastUserSync) > 30000)) {
            ...
            for (let key in onlineDict) {
                if (now - onlineDict[key].ts > 120000)
                    delete onlineDict[key];
            }
 ...
 */
function chatSendMsgCD(uid, rol, nombre, text, ghostMode = false, targetUid) {
    try {
        const threadUid = (rol.toLowerCase() === 'usuario') ? uid : targetUid;
        if (!threadUid)
            return { ok: false, error: "No se especificó destinatario" };
        ...
        let nextId = 1;
        if (msgsArr.length > 0)
            nextId = msgsArr[msgsArr.length - 1].id + 1;
        const nuevoMsg = {
            id: nextId,
            uid: uid,
            rol: rol,
        ...
        if (msgsArr.length > 60)
            msgsArr = msgsArr.slice(-60);
