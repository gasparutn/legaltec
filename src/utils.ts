 ...
 */
function logActivity(movimiento: string, activeAutor?: string) {
  try {
 ...
 */
function getColumnMap(sheet: GoogleAppsScript.Spreadsheet.Sheet): {[key: string]: number} {
  if (!sheet) return {};
...
function parseSafeDate(val: any): Date {
  ...
  if (!val || String(val).trim() === '') return new Date(NaN);
...
function fechaEnEspaniol(valor: string | Date): string {
  if (!valor) return '';
...
function replaceFields(target: GoogleAppsScript.Document.Body, fields: {[key: string]: any}): void {
 ...
 */
function replaceFullRow(body: GoogleAppsScript.Document.Body, data: {[key: string]: any}): void {
 ...
 */
function setCache(key: string, value: any, expirationInSeconds: number = 600): void {
  try {
