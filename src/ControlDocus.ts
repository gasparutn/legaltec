 ...
 */
function loginCD(usuario: string, pass: string): { ok: boolean; rol?: string; nombre?: string; uid?: string; message?: string } {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hoja = ss.getSheetByName(CD_HOJA_CONFIG);
    if (!hoja) return { ok: false, message: 'Hoja de configuración no encontrada.' };
...
function getSesionCD(): { rol: string; usuarioId: string; nombre: string } {
  const props = PropertiesService.getUserProperties();
  return {
...
function crearRequerimiento(apellidoNombre: string, autoridad: string, usuarioId?: string): { ok: boolean; message?: string; idReq?: number } {
  try {
    initHojasCD();
...
function listarRequerimientos(usuarioId?: string, rol?: string): { ok: boolean; data?: any[]; message?: string } {
  try {
    ...
    if (!uid) return { ok: false, message: 'No hay sesión activa. Por favor iniciá sesión nuevamente.' };

    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaReq = ss.getSheetByName(CD_HOJA_REQUERIMIENTOS);
    if (!hojaReq || hojaReq.getLastRow() < 2) return { ok: true, data: [] };
...
function getDocumentosReq(idReq: string): { ok: boolean; data?: any[]; message?: string } {
  try {
    ...
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaDoc = ss.getSheetByName(CD_HOJA_DOCUMENTOS);
    if (!hojaDoc || hojaDoc.getLastRow() < 2) return { ok: true, data: [] };
...
function _updateDocRow(hojaDoc: GoogleAppsScript.Spreadsheet.Sheet, idDoc: string, campos: { [key: number]: any }): boolean {
...
function subirDocumentoLocalCD(idDoc: string, mimeType: string, base64Data: string, fileName: string): { ok: boolean; message?: string } {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaDoc = ss.getSheetByName(CD_HOJA_DOCUMENTOS);
    if (!hojaDoc) return { ok: false, message: 'Hoja de documentos no encontrada.' };
    ...
    if (hojaReq) {
      ...
      for (const dr of docRows) {
        if (String(dr[0]) === String(idDoc)) { idReq = String(dr[1]); break; }
      }
      if (idReq) {
...
function aprobarDocumento(idDoc: string): { ok: boolean; message?: string } {
  try {
    const sesion = getSesionCD();
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaDoc = ss.getSheetByName(CD_HOJA_DOCUMENTOS);
    ...
    if (!ok) return { ok: false, message: 'Documento no encontrado.' };
...
function aprobarDocumentosLote(idDocs: string[]): { ok: boolean; message?: string } {
  try {
    const sesion = getSesionCD();
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaDoc = ss.getSheetByName(CD_HOJA_DOCUMENTOS);
    ...
    let reqsParaChequear = new Set<string>();
...
function _finalCheckCompletitudReqManual(hojaDoc: GoogleAppsScript.Spreadsheet.Sheet, ss: GoogleAppsScript.Spreadsheet.Spreadsheet, idReq: string): void {
  const rows = hojaDoc.getDataRange().getValues();
...
function _checkCompletitudReq(hojaDoc: GoogleAppsScript.Spreadsheet.Sheet, ss: GoogleAppsScript.Spreadsheet.Spreadsheet, idDoc: string): void {
  const rows = hojaDoc.getDataRange().getValues();
  // Buscar idReq del idDoc
  let idReq = '';
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0]) === String(idDoc)) { idReq = String(rows[i][1]); break; }
  }
  if (!idReq) return;
...
function rechazarDocumento(idDoc: string, motivo: string): { ok: boolean; message?: string } {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaDoc = ss.getSheetByName(CD_HOJA_DOCUMENTOS);
    if (!hojaDoc) return { ok: false, message: 'Hoja de documentos no encontrada.' };
...
function eliminarRequerimientoComp(idReq: string, uid: string, rol: string): { ok: boolean; message?: string } {
  try {
    ...
    const hojaReq = ss.getSheetByName(CD_HOJA_REQUERIMIENTOS);
    if (!hojaReq) return { ok: false, message: 'Hoja de requerimientos no encontrada.' };
    const reqRows = hojaReq.getDataRange().getValues();
    let rowReq = -1;
    for (let i = 1; i < reqRows.length; i++) {
 ...
 */
async function generarPdfCD(idReq: string): Promise<{ ok: boolean; urlPdf?: string; message?: string }> {
  try {
    const sesion = getSesionCD();
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    ...
    for (let i = 1; i < reqRows.length; i++) {
      if (String(reqRows[i][0]) === String(idReq)) { reqData = reqRows[i]; break; }
    }
    if (!reqData) return { ok: false, message: 'Requerimiento no encontrado.' };
...
function getPdfUrlCD(idReq: string): { ok: boolean; urlPdf?: string; message?: string } {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const hojaReq = ss.getSheetByName(CD_HOJA_REQUERIMIENTOS);
    if (!hojaReq) return { ok: false, message: 'Hoja no encontrada.' };
