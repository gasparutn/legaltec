...
function getServiceAccountToken() {
  ...
  if (res.error) throw new Error("Error obteniendo token: " + res.error_description);
 ...
 */
function firestoreCreateDocumentWithId(collection: string, docId: string, data: any) {
  ...
  for (const key in data) {
    const val = data[key];
    if (val === true || val === false) payload.fields[key] = { booleanValue: val };
    else if (val instanceof Date) payload.fields[key] = { timestampValue: val.toISOString() };
    else if (typeof val === 'number') payload.fields[key] = { doubleValue: val };
    else payload.fields[key] = { stringValue: String(val || "") };
  }
  ...
  if (respJson.error) throw new Error(`Error Firestore [${collection}/${docId}]: ${respJson.error.message}`);
  return respJson;
}
 ...
 */
function firestoreUpdateDocument(collection: string, docId: string, data: any) {
  const projectId = FIREBASE_CONFIG.project_id;
  const encodedId = encodeURIComponent(String(docId).trim());
...
function firestoreDeleteDocument(collection: string, docId: string) {
  const projectId = FIREBASE_CONFIG.project_id;
  const encodedId = encodeURIComponent(String(docId).trim());
  const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${collection}/${encodedId}`;
...
function firestoreGetDoc(collection: string, docId: string) {
  const projectId = FIREBASE_CONFIG.project_id;
  const encodedId = encodeURIComponent(String(docId).trim());
  const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${collection}/${encodedId}`;
...
function firestoreGetAllDocs(collection: string) {
  try {
    const projectId = FIREBASE_CONFIG.project_id;
    ...
    if (data.error) throw new Error(`Error Firestore: ${data.error.message}`);
 ...
 */
function firestoreToSheetSync(collectionName: string, sheetName: string) {
...
function migrateAnySheet(sheetName: string, collectionName: string, idColumnName: string) {
  const ss = getSS();
  const sheet = ss.getSheetByName(sheetName);
  if (!sheet) return;
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const rowObject = {};
    headers.forEach((h, idx) => { if (h) rowObject[h] = row[idx]; });
    let docId = String(rowObject[targetIdHeader] || "").trim();
    if (docId.endsWith(".0")) docId = docId.substring(0, docId.length - 2);
    if (!docId) docId = "row_" + i;
...
function onEdit(e: GoogleAppsScript.Events.SheetsOnEdit) {
  ...
  try {
    ...
    const rowObject = {};
    headers.forEach((h, idx) => { if (h) rowObject[h] = rowValues[idx]; });
    const targetIdHeader = normalizeHeader(config.idCol);
    let docId = String(rowObject[targetIdHeader] || "").trim();
    if (docId.endsWith(".0")) docId = docId.substring(0, docId.length - 2);
    if (docId) firestoreCreateDocumentWithId(config.col, docId, rowObject);
  } catch (err) {}
}
