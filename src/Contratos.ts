 ...
 */
function generaContratoWord(rowData: ConRowData): { url: string; nombre: string } {
  ...
  if (!apellidos) {
    const apellidoKeys = Object.keys(rowData).filter(k => 
      k.toUpperCase().includes('APELLIDO') && String(rowData[k]).trim() !== ''
    );
  ...
  if (!nombres) {
    const nombreKeys = Object.keys(rowData).filter(k => 
      k.toUpperCase().includes('NOMBRE') && String(rowData[k]).trim() !== ''
    );
...
function testGeneraContratoLocal() {
  ...
  try {
    const res = generaContratoWord(testData as any);
